...
create_user() {
  user_name=${1:?}
  group_name=${2:?}
  id "$user_name" >/dev/null 2>&1 || useradd -s /sbin/nologin -r -M "$user_name" -G "$group_name"
}
...
wait_pid() {
  ...
  if [ -e "/proc/$pid" ]; then
    ...
    while [ -e "/proc/$pid" ]; do
      sleep 0.1
      [ "$countdown" != '0' ] && [ $(( countdown % 10 )) = '0' ] && printf .
      if [ "$timeout" -gt 0 ]; then
        if [ $countdown -eq 0 ]; then
          if [ "$force" = "1" ]; then
            printf "\nKill timed out, using kill -9 on %s... " "$pid"
            kill -9 "$pid"
            sleep 0.5
          fi
          break
...
check_nfs_mount() {
  ...
  if grep -qs "$mount_point" /proc/mounts; then
    ...
    echo "Mounting NFS..."
    if ! mount $opts $exports $mount_point
    then
